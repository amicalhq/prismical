...
class FakeWorker extends EventEmitter {
  ...
  boot(): void {
    this.emit('message', {
      type: 'log',
      ...makeWire('info', 'whisper-worker', 'Worker process started'),
    });
  }
...
describe('WhisperEngine host', () => {
  it.effect(
    'forks lazily with the sidecar recipe on the first ensureModel; the model is cached by path',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
  ...
  it.effect(
    'transcribe serializes the Float32Array as { __type, data }, passes the options verbatim and yields the answer',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
        ...
        assert.deepStrictEqual(child.sent[1], {
          ...
          args: [
            { __type: 'Float32Array', data: [0.5, -0.25, 0.125] },
            { ...OPTIONS, vad: true },
          ],
        });
  ...
  it.effect(
    'a worker error reply is inference-failed (its message kept); transcribe before any ensureModel is inference-failed too',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
  ...
  it.effect(
    'a child that dies mid-call rejects it worker-crashed; the next call re-forks and re-loads the remembered model',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
        const init = yield* Effect.fork(h.engine.ensureModel(MODEL));
  ...
  it.effect(
    'a death before the first message (missing sidecar / whisper.node) is spawn-failed; a throwing fork too',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
        const init = yield* Effect.fork(h.engine.ensureModel(MODEL));
        ...
        child.emit(
          'error',
          Object.assign(new Error('spawn /fake/resources/node ENOENT'), { code: 'ENOENT' })
        );
        const error = yield* Fiber.join(init).pipe(Effect.flip);
  ...
  it.effect(
    'a call over budget is timeout; the stuck worker is killed and the next call re-forks',
    () =>
      Effect.gen(function* () {
        ...
        assert.isDefined(
          h.logger.find(
            e =>
              e.scope === 'whisper-engine' &&
              e.message === 'whisper worker call timed out — replacing the worker'
          )
        );
  ...
  it.effect(
    'log frames land on the whisper-worker scope at their level; scope close kills the child and rejects the pending call',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
        const init = yield* Effect.fork(h.engine.ensureModel(MODEL));
        ...
        child.emit('message', {
          type: 'log',
          ...makeWire('warn', 'whisper-worker', 'Segments processed', {
            context: { segmentCount: 3 },
            error: Object.assign(new Error('worker failure'), {
              code: 'ENOENT',
        ...
        assert.strictEqual(warn?.message, 'Segments processed');
        assert.strictEqual((warn?.data as { segmentCount: number }).segmentCount, 3);
        assert.strictEqual(warn?.runtime, 'worker');
        assert.strictEqual(warn?.pid, 1234);
        assert.strictEqual(warn?.error?.code, 'ENOENT');
        assert.strictEqual(warn?.error?.cause?.name, 'Error');
        assert.isUndefined(
          h.logger.find(e => e.message === 'odd level'),
  ...
  it.effect(
    'a failed model switch clears the cached path — the next call re-attempts the load and never decodes against the freed model',
    () =>
      Effect.gen(function* () {
        const h = yield* build();
        const init = yield* Effect.fork(h.engine.ensureModel(MODEL));
        ...
        assert.strictEqual(
          h.forks.length,
          1,
          'same worker throughout (a replied error never kills)'
        );
        yield* Scope.close(h.scope, Exit.void);
      })
  );
