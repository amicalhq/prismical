...
export function SiteHeader({
  ...
  const isCustomView = typeof currentView !== "string" && currentView != null;
  const mobileToolbarInsetWidth = "calc(var(--toolbar-left) + 1.75rem)";
    ...
    <>
      ...
      <header className="flex h-[var(--header-height)] shrink-0 items-center gap-2 backdrop-blur supports-[backdrop-filter]:bg-background sticky top-0 z-40 w-full">
        <div className="flex w-full items-center gap-1">
          <div
            className={`flex shrink-0 items-center gap-1 py-1.5 transition-[padding,width] duration-200 ${!isMobile && sidebarState === "expanded" ? "px-4" : "px-0"}`}
            style={isMobile ? { width: mobileToolbarInsetWidth } : undefined}
          />
          <div
            className={`transition-opacity duration-200 ${showTitle ? "opacity-100" : "opacity-0"} ${
              isCustomView
                ? "flex min-w-0 flex-1 items-center"
                : "flex items-center pointer-events-none select-none"
            }`}
          >
