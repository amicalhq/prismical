...
const skill = (
  ...
  system: false,
  enabled,
  config: { editingOptions: "replace-doc", surface, defaultSkill: false, modeAgnosticPrompt: false, askScope },
...
describe("askSkills", () => {
  ...
  it("hides single-note skills when no note is open", () => {
    const skills = [skill("multi", ["ask"], true, "multi-note"), skill("single", ["ask"], true, "single-note")];
