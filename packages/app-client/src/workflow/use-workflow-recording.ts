...
export function useWorkflowRecording(
  ...
  React.useEffect(() => {
    ...
    for (const queryKey of [
      transcriptKey(finished.recordingId),
      recordingsKey(finished.noteId),
      noteRecordingsKey(finished.noteId),
      enhancedRecordingsKey(finished.noteId),
      usageKeyPrefix,
    ]) {
  ...
  return {
    ...
    keepRecording: client.keepRecording,
    pauseFromPrompt: client.pauseFromPrompt,
    setLanguage: async (language: TranscriptionLanguage) => {
      // The account save can finish after this recording is replaced. Keep its original owner.
      if (!snapshot.recordingId || snapshot.recordingId !== client.getSnapshot().recordingId) return;
