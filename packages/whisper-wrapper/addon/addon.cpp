...
struct FullParamConfig {
  ...
  // whisper_full_params only holds a const char* for the VAD model path, so the
  // string lives here for the duration of the call (like initial_prompt).
...
FullParamConfig parse_full_params(const Napi::Env env, const Napi::Object& options) {
  ...
  // Built-in Silero VAD (whisper.cpp v1.8.x): only the detected speech is
  // decoded and whisper.cpp maps the returned segment timestamps back onto the
  // original timeline. vad_params starts from whisper_vad_default_params()
  // (set by whisper_full_default_params()); each field is overridable.
  if (options.Has("vad")) {
    cfg.params.vad = options.Get("vad").As<Napi::Boolean>().Value();
  }
  if (options.Has("vad_model_path") && options.Get("vad_model_path").IsString()) {
    cfg.vad_model_path = options.Get("vad_model_path").As<Napi::String>();
  }
  if (options.Has("vad_threshold")) {
    cfg.params.vad_params.threshold = options.Get("vad_threshold").As<Napi::Number>();
  }
  if (options.Has("vad_min_speech_duration_ms")) {
    cfg.params.vad_params.min_speech_duration_ms =
      options.Get("vad_min_speech_duration_ms").As<Napi::Number>().Int32Value();
  }
  if (options.Has("vad_min_silence_duration_ms")) {
    cfg.params.vad_params.min_silence_duration_ms =
      options.Get("vad_min_silence_duration_ms").As<Napi::Number>().Int32Value();
  }
  if (options.Has("vad_max_speech_duration_s")) {
    cfg.params.vad_params.max_speech_duration_s =
      options.Get("vad_max_speech_duration_s").As<Napi::Number>();
  }
  if (options.Has("vad_speech_pad_ms")) {
    cfg.params.vad_params.speech_pad_ms = options.Get("vad_speech_pad_ms").As<Napi::Number>().Int32Value();
  }
  if (options.Has("vad_samples_overlap")) {
    cfg.params.vad_params.samples_overlap = options.Get("vad_samples_overlap").As<Napi::Number>();
  }
...
Napi::Value full_transcribe(const Napi::CallbackInfo& info) {
  ...
  // whisper.cpp opens vad_model_path with std::ifstream unguarded; a null path
  // is undefined behaviour there, so refuse it here instead of crashing the worker.
  if (cfg.params.vad && cfg.vad_model_path.empty()) {
    throw Napi::TypeError::New(env, "vad requires vad_model_path");
  }
  cfg.params.vad_model_path = cfg.vad_model_path.empty() ? nullptr : cfg.vad_model_path.c_str();
