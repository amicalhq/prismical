...
import { nextAutoColor, useCreateTag, useTags } from '@prismical/app-client';
import { useAllNoteTags } from '@prismical/app-client';
import { useTranslation } from 'react-i18next';

const RECENT_LIMIT = 5;

/** Each "Show more" reveals this many further tags. */
const TAG_PAGE = 10;
...
export function NavTagsGroup() {
  ...
  // Same paging as the folders group: a page at a time, back to the short list
  // when the section is collapsed. Order stays newest-first throughout - the
  // old "show all" re-sorted by name, which would reshuffle rows under the
  // cursor on every click once the list grows in steps.
  const { shown, showMore } = useShowMore(open, RECENT_LIMIT, TAG_PAGE);
  const tags = newestFirst(allTags, shown);
  const moreTags = allTags.length > tags.length;
  const [createOpen, setCreateOpen] = useState(false);
  const createTag = useCreateTag();

  // Seeded the same way a tag typed on a note seeds it, so a tag created here lands on the same palette
        ...
        <CollapsibleContent>
          {loading ? (
            ...
            <SidebarMenu>
              ...
              {/* Under the list and always visible, like the folders group. */}
              {moreTags ? (
                <SidebarMenuItem>
                  <SidebarMenuButton
                    size="sm"
