...
export function PromptInputAttachment({ data, className, ...props }: PromptInputAttachmentProps) {
  ...
  const attachmentLabel =
    filename || (isImage ? t('ai.attachments.image') : t('ai.attachments.attachment'));
        ...
        >
          <div className="relative size-5 shrink-0">
            <div className="absolute inset-0 flex size-5 items-center justify-center overflow-hidden rounded bg-background transition-opacity group-hover:opacity-0">
              {isImage ? (
                <img
                  alt={filename || t('ai.attachments.attachmentAlt')}
            ...
            >
              <XIcon />
              <span className="sr-only">{t('ai.attachments.remove')}</span>
            </Button>
          </div>
        ...
        <div className="w-auto space-y-3">
          {isImage && (
            <div className="flex max-h-96 w-96 items-center justify-center overflow-hidden rounded-md border">
              <img
                alt={filename || t('ai.attachments.previewAlt')}
...
export const PromptInputActionAddAttachments = ({
  ...
  return (
    ...
    >
      <ImageIcon className="mr-2 size-4" /> {label ?? t('ai.attachments.add')}
    </DropdownMenuItem>
  );
};
  ...
  const addLocal = useCallback(
    (fileList: File[] | FileList) => {
      ...
      if (incoming.length && accepted.length === 0) {
        onError?.({
          code: 'accept',
          message: t('ai.attachments.errors.accept'),
        });
        return;
      }
      ...
      if (accepted.length > 0 && sized.length === 0) {
        onError?.({
          code: 'max_file_size',
          message: t('ai.attachments.errors.maxFileSize'),
        });
        return;
      }
      ...
      setItems(prev => {
        ...
        if (typeof capacity === 'number' && sized.length > capacity) {
          onError?.({
            code: 'max_files',
            message: t('ai.attachments.errors.maxFiles'),
          });
        }
...
export const PromptInputSpeechButton = ({
  ...
  useEffect(() => {
    if (
      ...
      speechRecognition.lang = resolvedLocale;
  ...
  }, [onTranscriptionChange, resolvedLocale, textareaRef]);
  ...
  return (
    <PromptInputButton
      aria-label={isListening ? t('ai.speech.stop') : t('ai.speech.start')}
      className={cn(
