...
import { useFolders, useNotes } from '@prismical/app-client';
import { formatApplicationLastMet, useApplicationLocale } from '@prismical/app-i18n';
import { useTranslation } from 'react-i18next';

/**
 * "Shared with me": the folders and notes the caller can read but does not own. Derived from
 * the lists' `isOwner` flags — no extra fetch. Where an accepted invite lands. Folders come
 * first: a shared folder is the bigger grant. Its notes are inbound too, so they also appear in
...
export function SharedScreen() {
  ...
  // Only the roots of what was shared: access reaches every folder below a shared one, so its
  // subfolders arrive as inbound rows too, and listing them would repeat the tree here.
  const visible = new Set(folders.map(f => f.id));
  const sharedFolders = folders.filter(
    f => f.isOwner === false && !(f.parentId && visible.has(f.parentId))
  );
  ...
  return (
    <div className="mx-auto w-full" style={{ maxWidth: 'var(--content-width-browse)' }}>
      ...
      {isLoading ? (
        <ListRowsSkeleton />
      ) : shared.length === 0 && sharedFolders.length === 0 ? (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="truncate text-xs text-muted-foreground">
                        {t('shared.sharedBy', { name: folder.sharedByName || t('shared.someone') })}
                      </p>
                    </div>
                  </Link>
                ))}
