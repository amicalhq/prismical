...
internal sealed record MicInputDevice(
...
internal sealed record MicActiveApp(
    ...
    [property: JsonPropertyName("inputDevices")] List<MicInputDevice> InputDevices
);
        ...
        foreach (var device in deviceEnumerator.EnumerateAudioEndPoints(DataFlow.Capture, DeviceState.Active))
        {
            try
            {
                ...
                for (var index = 0; index < sessions.Count; index += 1)
                {
                    ...
                    if (appsByPid.TryGetValue(pid, out var existing))
                    {
                        if (!existing.InputDevices.Any(input => input.Uid == device.ID))
                        {
                            existing.InputDevices.Add(
                                new MicInputDevice(device.ID, device.FriendlyName)
                            );
                        }
                        continue;
                    }
