...
func getInputDeviceIDs() throws -> [AudioDeviceID] {
    let deviceIDs: [AudioDeviceID] = try getAudioObjectArrayProperty(
        objectID: AudioObjectID(kAudioObjectSystemObject),
    ...
    return deviceIDs.filter { deviceID in
        let streams: [AudioObjectID]? = try? getAudioObjectArrayProperty(
            objectID: deviceID,
...
func getInputDeviceID(uid: String) throws -> AudioDeviceID? {
    for deviceID in try getInputDeviceIDs() {
        let deviceUID = try? getAudioObjectStringProperty(
            objectID: deviceID,
            selector: kAudioDevicePropertyDeviceUID
        )
        if deviceUID == uid {
...
func getInputDeviceDetails(deviceID: AudioDeviceID) throws -> (uid: String, name: String) {
    let uid = try getAudioObjectStringProperty(
        objectID: deviceID,
        selector: kAudioDevicePropertyDeviceUID
    )
    let name = try getAudioObjectStringProperty(
        objectID: deviceID,
...
final class AudioObjectPropertyObservation {
    ...
    init(
        ...
        self.address = makePropertyAddress(selector: selector, scope: scope, element: element)
        self.queue = queue
        self.listener = { _, _ in onChange() }

        try coreAudioCheck(
            AudioObjectAddPropertyListenerBlock(objectID, &address, queue, listener),
            operation: "AudioObjectAddPropertyListenerBlock(\(selector))"
        )
    }
    ...
    func cancel() {
        ...
        if status != noErr {
            Logger.error("AudioObjectRemovePropertyListenerBlock failed: \(status)")
        }
    }
