...
final class SystemAudioCapture {
    ...
    var aggregateSampleRateObservation: AudioObjectPropertyObservation?
    ...
    func start() async throws {
        ...
        self.aggregateSampleRateObservation = try AudioObjectPropertyObservation(
            objectID: aggregateDeviceID,
            selector: kAudioDevicePropertyNominalSampleRate,
            queue: processingQueue
        ) { [weak self] in
            self?.refreshAggregateDeviceSampleRate()
        }
    ...
    func stop() async {
        ...
        aggregateSampleRateObservation?.cancel()
        aggregateSampleRateObservation = nil
    ...
    private func refreshAggregateDeviceSampleRate() {
        guard aggregateDeviceID != kAudioObjectUnknown else { return }
        guard let sampleRate: Float64 = try? getAudioObjectProperty(
            objectID: aggregateDeviceID,
            selector: kAudioDevicePropertyNominalSampleRate,
            type: Float64.self
        ), sampleRate > 0 else { return }

        let previousSampleRate = aggregateDeviceSampleRate
        aggregateDeviceSampleRate = sampleRate

        if let previousSampleRate, abs(previousSampleRate - sampleRate) >= 0.5 {
            Logger.info(
                "System audio aggregate sample rate changed: from=\(Int(previousSampleRate)) to=\(Int(sampleRate))"
            )
            traceWriter?.record(
                event: "system_aggregate_sample_rate_changed",
                metadata: [
