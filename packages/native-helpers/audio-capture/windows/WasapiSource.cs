...
internal sealed class WasapiSource : IDisposable
{
    ...
    private readonly Action<Exception>? onUnexpectedStop;
    ...
    private long processedSourceSampleCount;
    private long emittedOutputSampleCount;
    ...
    public WasapiSource(
        ...
        Action<Exception>? onUnexpectedStop = null
    )
    {
        this.kind = kind;
    ...
    private void HandleDataAvailable(object? sender, WaveInEventArgs eventArgs)
    {
        try
        {
            ...
            processedSourceSampleCount += monoSamples.Length;
            var expectedOutputSampleCount = (long)Math.Round(
                processedSourceSampleCount * (double)CaptureConstants.SampleRate / inputSampleRate,
                MidpointRounding.AwayFromZero
            );
            var outputSampleCount = Math.Max(
                0,
            ...
            emittedOutputSampleCount += resampledSamples.Length;
    ...
    private void HandleRecordingStopped(object? sender, StoppedEventArgs eventArgs)
    {
        if (eventArgs.Exception != null)
        {
            ...
            {
                _ = Task.Run(() => onUnexpectedStop(exception));
            }
        }
    }
