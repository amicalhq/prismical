...
internal static class Logger
{
    ...
    {
        Console.Error.WriteLine($"mic-event={JsonSerializer.Serialize(message, JsonOptions)}");
    }
}
...
)
{
    public static ParsedArguments Parse(string[] args)
    {
        ...
        for (var index = 0; index < args.Length; index += 1)
        {
            ...
            if (args[index] == "--mic-device" && index + 1 < args.Length)
            {
                microphoneDeviceId = args[index + 1];
                index += 1;
                continue;
            }
